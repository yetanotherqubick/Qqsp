...
QspReply::QspReply(const QUrl &url, const QString &_text, bool _isUseHtml, const QString &_path, const QColor &_linkColor,
                   const QColor &_backColor, const QColor &_fontColor, const QString &_bmpBg, const QFont &_font)
{
    offset = 0;
    m_text = _text;
    m_path = _path;
    ...
    if (url_str.compare("qsp:", Qt::CaseInsensitive) == 0 || url_str.compare("qsp:/", Qt::CaseInsensitive) == 0)
    {
        ...
        if (m_backColor.isValid()) replystr.append(QString("background-color: %1;\n").arg(m_backColor.name()));
        if (m_fontColor.isValid()) replystr.append(QString("color: %1;\n").arg(m_fontColor.name()));
        ...
        if (m_linkColor.isValid()) replystr.append(QString("}\na:link {\ncolor: %1;\n").arg(m_linkColor.name()));
        replystr.append("}\n</style></head>\n<body>\n");
