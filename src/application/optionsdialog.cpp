    ...
    ui(new Ui::OptionsDialog)
{
    ...
    ui->label_backColor->setStyleSheet(QStringLiteral("background-color: rgb(%1, %2, %3);").arg(QString::number(m_backColor.red()), QString::number(m_backColor.green()), QString::number(m_backColor.blue())));
    ui->label_linkColor->setStyleSheet(QStringLiteral("background-color: rgb(%1, %2, %3);").arg(QString::number(m_linkColor.red()), QString::number(m_linkColor.green()), QString::number(m_linkColor.blue())));
    ui->label_fontColor->setStyleSheet(QStringLiteral("background-color: rgb(%1, %2, %3);").arg(QString::number(m_fontColor.red()), QString::number(m_fontColor.green()), QString::number(m_fontColor.blue())));
    ...
    ui->horizontalSlider_volume->setValue(static_cast<int>(mw->GetOverallVolume() * 100));
    ...
    QDirIterator qmIt(QCoreApplication::applicationDirPath(), {QStringLiteral("*.qm")}, QDir::Files);
    while (qmIt.hasNext())
    {
        qmIt.next();
        const QFileInfo finfo = qmIt.fileInfo();
    ...
    QDirIterator qmItRC(QStringLiteral(":/translations/"), {QStringLiteral("*.qm")}, QDir::Files);
    while (qmItRC.hasNext())
    {
        qmItRC.next();
        const QFileInfo finfo = qmItRC.fileInfo();
        QTranslator translator;
        if (translator.load(finfo.completeBaseName(), finfo.absolutePath()))
            ui->comboBox_language->addItem(translator.translate("__LANG__", "__LANGNAME__"), QVariant(translator.translate("__LANG__", "__LANGID__")));
    }
    langid = mw->GetLangID();
    const int index = ui->comboBox_language->findData(langid);
    if (index != -1)
