...
bool VideoFrameProcessor::present(const QVideoFrame &frame)
{
    if (frame.isValid())
    {
        ...
        if (videoFrame.map(QAbstractVideoBuffer::ReadOnly))
        {
            ...
            Q_EMIT newFrame(QImage(videoFrame.bits(), videoFrame.width(), videoFrame.height(), videoFrame.bytesPerLine(),
                                   QVideoFrame::imageFormatFromPixelFormat(videoFrame.pixelFormat())));
        }
        videoFrame.unmap();
    }
...
QList<QVideoFrame::PixelFormat> VideoFrameProcessor::supportedPixelFormats(QAbstractVideoBuffer::HandleType handleType) const
{
    ...
    return QList<QVideoFrame::PixelFormat>() << QVideoFrame::Format_RGB32 << QVideoFrame::Format_ARGB32
                                             << QVideoFrame::Format_ARGB32_Premultiplied << QVideoFrame::Format_RGB565
