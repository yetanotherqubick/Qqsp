...
bool QspTextBox::SetLinkColor(const QColor &color)
{
    ...
    // Note that we do not use the Link and LinkVisited roles when rendering rich text in Qt, and that we recommend that you use CSS and the
    // QTextDocument::setDefaultStyleSheet() function to alter the appearance of links.
...
// The meaning of foreground colour varies according to the window class; it may be the text colour or other colour, or it may not be used
// at all. Additionally, not all native controls support changing their foreground colour so this method may change their colour only
...
void QspTextBox::CalcImageSize()
{
    if (!m_bmpBg.isNull())
    {
        ...
        m_posX = (w - destW) / 2;
        m_posY = (h - destH) / 2;
        if (destW > 0 && destH > 0) m_bmpRealBg = m_bmpBg.scaled(destW, destH);
    }
}
...
void QspTextBox::clearManualResources()
{
    ...
    {
        if (animationsItem.movie != 0) delete animationsItem.movie;
        if (animationsItem.movieLabel != 0) delete animationsItem.movieLabel;
    }
    animations_gif.clear();
    for (auto animationsItem : animations_video)
    {
        if (animationsItem.videoLabel != 0) delete animationsItem.videoLabel;
    }
...
QVariant QspTextBox::loadResource(int type, const QUrl &name)
{
    ...
    if (new_name.endsWith(".gif", Qt::CaseInsensitive) || new_name.endsWith(".mng", Qt::CaseInsensitive))
    {
        ...
        {
            if (movie != 0) delete movie;
            if (videoL != 0) delete videoL;
            QImage image(1, 1, QImage::Format_ARGB32);
            image.fill(qRgba(0, 0, 0, 0));
            return QVariant(image);
        }
    }
    if (new_name.endsWith(".mp4", Qt::CaseInsensitive) || new_name.endsWith(".avi", Qt::CaseInsensitive) ||
        new_name.endsWith(".wmv", Qt::CaseInsensitive) || new_name.endsWith(".mkv", Qt::CaseInsensitive) ||
        new_name.endsWith(".webm", Qt::CaseInsensitive) || new_name.endsWith(".m4v", Qt::CaseInsensitive) ||
        new_name.endsWith(".ogv", Qt::CaseInsensitive))
    {
        if (!disableVideo)
        {
                        ...
                        if (it.fragment().charFormat().isImageFormat())
                        {
                            ...
                            if (animations_gif.contains(it.fragment().charFormat().toImageFormat().name()))
                            {
                                ...
                                if (it.fragment().charFormat().toImageFormat().width() > 0 &&
                                    it.fragment().charFormat().toImageFormat().height() > 0)
                                    animations_gif[it.fragment().charFormat().toImageFormat().name()].movieLabel->setGeometry(
                                        curRect.x(), curRect.y(), it.fragment().charFormat().toImageFormat().width(),
                                        it.fragment().charFormat().toImageFormat().height());
                                else
                                    animations_gif[it.fragment().charFormat().toImageFormat().name()].movieLabel->move(curRect.x(),
                                                                                                                       curRect.y());
                            }
                            else if (animations_video.contains(it.fragment().charFormat().toImageFormat().name()))
                            {
                                if (animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel != 0)
                                {
                                    if (animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->hasFrame())
                                    {
                                        ...
                                        if (it.fragment().charFormat().toImageFormat().width() > 0 &&
                                            it.fragment().charFormat().toImageFormat().height() > 0)
                                            animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->setGeometry(
                                                curRect.x(), curRect.y(), it.fragment().charFormat().toImageFormat().width(),
                                                it.fragment().charFormat().toImageFormat().height());
                                        else
                                        {
                                            animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->move(
                                                curRect.x(), curRect.y());
                                            if (!animations_video[it.fragment().charFormat().toImageFormat().name()]
                                                     .videoLabel->resolution_set)
                                            {
                                                QTextImageFormat newImageFormat = it.fragment().charFormat().toImageFormat();
                                                newImageFormat.setWidth(animations_video[it.fragment().charFormat().toImageFormat().name()]
                                                                            .videoLabel->getResolution()
                                                                            .width());
                                                newImageFormat.setHeight(animations_video[it.fragment().charFormat().toImageFormat().name()]
                                                                             .videoLabel->getResolution()
                                                                             .height());
                                                if (newImageFormat.isValid())
                                                {
                                                    ...
                                                    cur.setPosition(it.fragment().position() + it.fragment().length(),
                                                ...
                                                animations_video[it.fragment().charFormat().toImageFormat().name()].videoLabel->setGeometry(
                                                    curRect.x(), curRect.y(),
                                                    animations_video[it.fragment().charFormat().toImageFormat().name()]
                                                        .videoLabel->getResolution()
                                                        .width(),
                                                    animations_video[it.fragment().charFormat().toImageFormat().name()]
                                                        .videoLabel->getResolution()
                                                        .height());
                                                animations_video[it.fragment().charFormat().toImageFormat().name()]
                                                    .videoLabel->resolution_set = true;
                                            }
                                        }
                                    }
                                }
